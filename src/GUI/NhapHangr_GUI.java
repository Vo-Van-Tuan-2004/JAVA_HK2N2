...
//thêm mã nhân viên đang đăng nhập
        nvField.setText(user.getTenTaiKhoan());
...
// hanh dong cho nut xac nhan
        xuatButton.addActionListener(e ->{
            ...
            List<ChiTietPhieuNhap_DTO> chitietList = new ArrayList<>();
            for (int i = 0; i < phieunhap_dtm.getRowCount(); i++) {
                String[] row = new String[phieunhap_dtm.getColumnCount()];
                for (int j = 0; j < phieunhap_dtm.getColumnCount(); j++) {
                    row[j] = phieunhap_dtm.getValueAt(i, j).toString();
                }
                ChiTietPhieuNhap_DTO chitiet = new ChiTietPhieuNhap_DTO();
                chitiet.setMa_phieu_nhap(row[0]);
                chitiet.setMa_san_pham(row[1]);
                chitiet.setSo_luong_nhap(Integer.parseInt(row[2]));
                chitiet.setGia_nhap(Integer.parseInt(row[3]));
                chitietList.add(chitiet);
            }   

            new XuatPhieuNhapr_GUI(phieunhap, phieunhap_dtm, chitietList);
